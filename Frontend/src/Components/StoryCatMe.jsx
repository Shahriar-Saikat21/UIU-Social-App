...
const showMyStory = async () => {
  return axios.get("http://localhost:3000/showMyStory",{
    withCredentials: true,
  });
};

async function deletePost(id){
    const data = await axios.get(`http://localhost:3000/deletePost/${id}`,{
      withCredentials: true,
    })
            ...
            {data?.data?.story.map((item)=>{
                ...
                <div className="flex justify-start items-center p-2 w-full h-auto">
                  ...
                  <div className="ml-2 flex flex-col w-full justify-center items-start">
                    ...
                    <button className="text-[#FD7E14] hover:text-gray-400" onClick={()=>deletePost(item.s_id)}>Delete</button>
                  </div>
                </div>
